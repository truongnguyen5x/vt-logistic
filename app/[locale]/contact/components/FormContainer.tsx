...
const FormContainer: FC<PropsContent> = ({ title, placeholder, locale }) => {
  return (
    <ApolloNextAppProvider
      makeClient={makeClient}
      makeSuspenseCache={makeSuspenseCache}
    >
      <FormContent title={title} placeholder={placeholder} locale={locale} />
    </ApolloNextAppProvider>
  );
};
...
const FormContent: FC<PropsContent> = ({ title, placeholder, locale }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
            ...
            render={({ field }) => (
              <input
                className={`bg-th-gray-220 h-14 focus-visible:outline-none px-6 py-4 w-[327px] ${
                  !!errors.title?.message ? "border border-th-red-500" : ""
                }`}
        ...
        <div className="animation">
          ...
          <Controller
            ...
            render={({ field }) => (
              <input
                className={`bg-th-gray-220 h-14 focus-visible:outline-none px-6 py-4 w-[327px] ${
                  !!errors.fullname?.message ? "border border-th-red-500" : ""
                }`}
        ...
        <div className="animation">
          ...
          <Controller
            ...
            render={({ field }) => (
              <input
                className={`bg-th-gray-220 h-14 focus-visible:outline-none px-6 py-4 w-[327px] ${
                  !!errors.email?.message ? "border border-th-red-500" : ""
                }`}
          ...
          render={({ field }) => (
            <input
              className={`bg-th-gray-220 h-14 w-full focus-visible:outline-none px-6 py-4 xl:w-[685px] ${
                !!errors.address?.message ? "border border-th-red-500" : ""
              }`}
          ...
          render={({ field }) => (
            <textarea
              className={`bg-th-gray-220 focus-visible:outline-none px-6 py-4 w-full ${
                !!errors.content?.message ? "border border-th-red-500" : ""
              }`}
              rows={4}
