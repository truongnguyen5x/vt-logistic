...
export const Card: FC<CardProps> = ({ data, className = "" }) => {
  const t = useTranslations("news");

  return (
    <Link href={data.link} className={className}>
      <Image src={data.image} alt="" width={940} height={360} />
        ...
        <div className="flex gap-8 items-start">
          <div className="min-w-[60px]">
            ...
            <div className="text-center mt-2 text-4xl text-th-gray-320 font-bold">
              {getDate(new Date(data.created_at)).toLocaleString("en-US", {
                minimumIntegerDigits: 2,
                useGrouping: false,
              })}
            </div>
            <div className="text-center font-medium text-base text-th-gray-300">{`Tháng ${
              getMonth(new Date(data.created_at)) + 1
            }`}</div>
          </div>
...
export const SideCard: FC<CardProps> = ({ data, className }) => {
  return (
    <Link href={data.link} className={className}>
      <Image src={data.image} alt="" width={490} height={240} />
      <div className="pt-6 px-8 pb-8 bg-th-gray-220 max-w-[490px]">
        ...
        <h5 className="text-th-gray-320 text-xl font-semibold">{data.title}</h5>
      </div>
    </Link>
  );
};
